...
//        self.navigationItem.title = self.generalFunc.getLanguageLabel(origValue: "", key: "LBL_ACCOUNT_VERIFY_TXT")
    ...
    func openEditView(editType:String){
        ...
        let heightOfDescription = descriptionStr.height(withConstrainedWidth: Application.screenSize.width - 20, font: UIFont(name: "Roboto-Light", size: 15)!) - 18
        ...
        if(editType == "MOBILE"){
            ...
            //Marlon - Propriedade não existe mais na nova versão do componente
            //self.editBoxTxtField.getTextField()!.isDetailLabelPaddingLocked = true
            
            self.countryLbl.text = "+\(userProfileJson.get("vPhoneCode"))"
            let countryLblWidth = self.countryLbl.text!.width(withConstrainedHeight: 21, font: UIFont(name: "Roboto-Light", size: 17)!)
            //TODO: Verificar outra maneira de colocar padding no texto
//            self.editBoxTxtField.textPadding = UIEdgeInsets(top: 0, left: countryLblWidth + 27, bottom: 0, right: 0)
