...
//        Utils.printLog(msgData: "Begin Editing")
    ...
    func fetchAutoCompletePlaces(searchText:String){
        //Marlon - Alterada quantidade mínima de letras necessárias para busca de endereço
...
//            changeContentSize(PAGE_HEIGHT: Application.screenSize.height - 100)
        ...
        var autoCompleteUrl = "https://maps.googleapis.com/maps/api/place/autocomplete/json?input=\(searchText)&key=\(Configurations.getGoogleServerKey())&language=\(Configurations.getGoogleMapLngCode())&sensor=true"
    ...
    func checkRecentPlaces(){
        ...
        for i in 0..<self.dataArrList.count{
            ...
            let height = address!.height(withConstrainedWidth: Application.screenSize.width - 77, font: UIFont(name: "Roboto-Light", size: 16)!)
    ...
    func findPlaceDetail(placeId:String, description:String){
        ...
        let placeDetailUrl = "https://maps.googleapis.com/maps/api/place/details/json?placeid=\(placeId)&key=\(Configurations.getGoogleServerKey())&language=\(Configurations.getGoogleMapLngCode())&sensor=true"
