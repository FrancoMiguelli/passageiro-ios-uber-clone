...
class LoadAvailableCab: NSObject, OnTaskRunCalledDelegate {
    ...
    func changeCabs(){
        /*if(driverMarkerList.count > 0 && !self.mainScreenUv.isPosPayment){
    ...
    func checkAvailableCabs(){
        ...
        if(mainScreenUv != nil && mainScreenUv.requestPickUpView != nil){
            ...
            mainScreenUv.requestNowBtn.isAppTheme = true
    ...
    func getFeaturedDriverList() -> [NSDictionary]{
        let driverList = self.listOfDrivers
    ...
    func filterDrivers(isCheckAgain:Bool){
        ...
        if listOfDrivers.count > 0{
            if(self.selctedFilterType == "eIsFeatured"){
                listOfDrivers = self.getFeaturedDriverList()
            }else if(self.selctedFilterType == "vAvgRating"){
                listOfDrivers = listOfDrivers.sorted(by: { $0.getFloat("average_rating") > $1.getFloat("average_rating")})
            }else if(self.selctedFilterType == "distance"){
                listOfDrivers =  listOfDrivers.sorted(by: { $0.getFloat("DIST_TO_PICKUP") < $1.getFloat("DIST_TO_PICKUP")})
            }
        }
        ...
        for i in 0..<listOfDrivers.count{
            ...
            if(self.selectedCabTypeId == "" || vCarType.contains(self.selectedCabTypeId) == false || (self.mainScreenUv.currentCabGeneralType == Utils.cabGeneralType_Ride && self.mainScreenUv.isHandicapPrefEnabled == true && eHandiCapAccessibility.uppercased() != "YES") || (self.mainScreenUv.currentCabGeneralType == Utils.cabGeneralType_Ride && eFemaleOnlyReqAccept.uppercased() == "YES" && self.mainScreenUv.userProfileJson.get("eGender").uppercased() != "FEMALE") || (self.mainScreenUv.currentCabGeneralType == Utils.cabGeneralType_Ride && DriverGender.uppercased() != "FEMALE" && self.mainScreenUv.isPreferFemaleDriverEnable == true) || (self.mainScreenUv.ufxSelectedServiceProviderId != "" && self.mainScreenUv.ufxSelectedServiceProviderId != driverData.get("driver_id")) || ((self.mainScreenUv.isCashPayment == true && driverData.get("ACCEPT_CASH_TRIPS") == "No" && self.mainScreenUv.currentCabGeneralType.uppercased() != Utils.cabGeneralType_UberX.uppercased()) || (self.mainScreenUv.currentCabGeneralType.uppercased() == Utils.cabGeneralType_UberX.uppercased() && driverData.get("ACCEPT_CASH_TRIPS") == "No" && self.mainScreenUv.userProfileJson.get("APP_PAYMENT_MODE").uppercased() == "CASH"))){
                continue
            }
    ...
    func drawMarker(location:CLLocation, Name:String, driverData:NSDictionary) -> GMSMarker{
        ...
        //Marlon
        driverMarker.icon = driverMarker.icon?.resize(toWidth: 50)
        driverMarker.icon = driverMarker.icon?.resize(toHeight: 50)
