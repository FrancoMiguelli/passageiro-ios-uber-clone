...
//        }
    ...
    func setData(){
        ...
        self.signInHLbl.font = RobotoFont.bold(with: 15)
        self.forgetPasswordLbl.font = RobotoFont.bold(with: 14)
        self.goSignUpLbl.font = RobotoFont.bold(with: 20)
...
//        self.orLbl.transform = CGAffineTransform(rotationAngle: -CGFloat(CGFloat.pi) / 2)
        ...
        self.signInBtn.isAppTheme = true
        ...
        //Marlon - Comentado aqui pois no app Android não tem este texto
        self.userNameTxtFieldHelperLbl.alpha = 0
//        self.userNameTxtFieldHelperLbl.text = self.generalFunc.getLanguageLabel(origValue: "Enter without leading + or 00 or country code", key: "LBL_SIGN_IN_MOBILE_EMAIL_HELPER")
        ...
        let helperTxtHeight = self.userNameTxtFieldHelperLbl.text!.height(withConstrainedWidth: Application.screenSize.width - 40, font: UIFont(name: "Roboto-Light", size: 14)!)
    ...
    func onTextChanged(sender: MyTextField, text: String) {
        sender.text = text
        sender.custextColor = UIColor.black
