...
class ViewProfileUV: UIViewController, MyLabelClickDelegate {
    ...
    func setData(){
        ...
        //Verifica se há imagem de usuário no JSON
        if (userProfileJson.get("vImgName") == "") {
            let imageData = GeneralFunctions.getValue(key: Utils.USER_IMAGE_KEY)
            
            if (imageData != nil) {
                let userImage = UIImage(data: imageData as! Data)
                usrProfileImgView.image = userImage
            }
            ...
            GeneralFunctions.removeValue(key: Utils.USER_IMAGE_KEY)
        }
