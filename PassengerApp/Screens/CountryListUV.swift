...
//        self.navigationItem.title = self.generalFunc.getLanguageLabel(origValue: "", key: "LBL_SELECT_CONTRY")
    ...
    func setDefaultNavBar(){
        self.addBackBarBtn()
        //TODO: Verificar se o botão ficou no estilo necessário, pois a sintaxe do método foi alterada
//        let rightButton: UIBarButtonItem = UIBarButtonItem(barButtonSystemItem: UIBarButtonItem.Style.plain, target: self, action: #selector(self.searchTapped))
        let rightButton: UIBarButtonItem = UIBarButtonItem(title: nil, style: UIBarButtonItem.Style.plain, target: self, action: #selector(self.searchTapped))
    ...
    @objc func keyboardWillAppear(sender: NSNotification){
        let keyboardSize = ([UIResponder.keyboardFrameEndUserInfoKey] as! NSValue).cgRectValue.height
        
        self.tableView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: keyboardSize, right: 0)
    }
...
//        indexTitle
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        ...
        Utils.printLog(msgData: "searchQuery::\(searchQuery)")
