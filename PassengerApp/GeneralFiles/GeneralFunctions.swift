...
class GeneralFunctions: NSObject {
    ...
    func getPlistValue(key:String, fileName:String) -> String{
    ...
    func getLanguageLabel(origValue:String, key:String) -> String{
        ...
        /*if(languageLabels == nil){
         ...
         var lngValue = languageLabels!.get(key)
         
         if(lngValue == ""){
         return (origValue == "" ? (key.hasPrefix("LBL_") ? origValue : key) : origValue)
         }*/
        let lngValue = getPlistValue(key: key, fileName: "strings")
        return lngValue
    }
    ...
    func setError2(uv:UIViewController?, title:String, content:String){
        DispatchQueue.main.async() {
            //            NBMaterialAlertDialog.showAlertWithTextAndTitle(uv.view, isCancelable: true, text: content, title: title, dialogHeight: nil, okButtonTitle: self.getLanguageLabel(origValue: "OK", key: "LBL_BTN_OK_TXT"), action: nil, cancelButtonTitle: nil)
    ...
    func loadView(nibName:String, uv:UIViewController, contentView:UIView) -> UIView {
        ...
        if(uv.navigationController != nil){
            ...
            uv.navigationController!.navigationBar.titleTextAttributes = [NSAttributedString.Key.font: RobotoFont.bold(with: 20), NSAttributedString.Key.foregroundColor: UIColor.white]
        }
    ...
    func addMDloader(contentView:UIView) -> UIView{
        ...
        let mdLoaderView: MDProgress?
        mdLoaderView = parentView.subviews[0] as? MDProgress
    ...
    func showLoader(view:UIView) -> NBMaterialLoadingDialog{
        let loadingDialog = NBMaterialLoadingDialog.showLoadingDialogWithText(view, message: (GeneralFunctions()).getLanguageLabel(origValue: "Loading", key: "LBL_LOADING_TXT"))
    ...
    static func isTripStatusMsgExist(_ msgDataDict:NSDictionary, _ isGenerateLocalNotification:Bool) -> Bool{
        Utils.printLog(msgData: "msgDataDict::\(msgDataDict)")
        if(msgDataDict.get("iTripId") != "" && msgDataDict.get("MsgType").uppercased() != "CHAT" && msgDataDict.get("Message") != ""){
