...
//                            cell.statusVLbl.text = self.generalFunc.getLanguageLabel(origValue: "", key: "LBL_CANCELED_TXT")
            ...
            cell.statusVLbl.textColor = UIColor.UCAColor.AppThemeColor
            ...
            let secondText = cell.statusVLbl.text!
            cell.statusVLbl!.textColor = UIColor.UCAColor.AppThemeColor
            cell.statusVLbl.textAlignment = .center
            cell.statusVLbl.text =  "\(statusH_str)\(cell.statusVLbl.text!)"
            
            let myString = cell.statusVLbl!.text! as NSString
            var myMutableString = NSMutableAttributedString()
            
            myMutableString = NSMutableAttributedString(string: myString as String, attributes: [NSAttributedString.Key.font: RobotoFont.bold(with: 16)])
            
            var textCount = statusH_str.count - 1
            
            myMutableString.addAttribute(NSAttributedString.Key.font, value: RobotoFont.bold(with: 16), range: NSRange(location: 1, length: textCount))
            
            textCount = statusH_str.count
            let cellCount = secondText.count - 1
            
            myMutableString.addAttribute(NSAttributedString.Key.font, value: RobotoFont.medium(with: 16), range: NSRange(location: textCount, length: cellCount))
            
            // set label Attribute
            cell.statusVLbl.attributedText = myMutableString
