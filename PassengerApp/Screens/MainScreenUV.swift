...
class MainScreenUV: UIViewController, GMSMapViewDelegate, OnLocationUpdateDelegate, AddressFoundDelegate, UICollectionViewDelegate, UICollectionViewDataSource , MyBtnClickDelegate, OnDirectionUpdateDelegate, OnTaskRunCalledDelegate, UITableViewDelegate, UITableViewDataSource , UICollectionViewDelegateFlowLayout , MyLabelClickDelegate{
    ...
    @IBOutlet weak var machinePaymentView: UIView!
    @IBOutlet weak var cashPayLbl: MyLabel!
    @IBOutlet weak var cardPayLbl: MyLabel!
    @IBOutlet weak var machinePayLbl: MyLabel!
    ...
    @IBOutlet weak var arrowPayImgView: UIImageView!
    ...
    var isPosPayment = false
    var isCardPayment = false
    var isPosPaymentEmpty = false
    ...
    override func viewDidLoad() {
        ...
        let PUBNUB_DISABLED = GeneralFunctions.getValue(key: Utils.PUBNUB_DISABLED_KEY) as! String
        ...
        if(self.APP_TYPE.uppercased() != "UBERX"){
            ...
            self.filterLbl.font = UIFont(name: "Roboto-Bold", size: 16.0)
    ...
    @objc func filterTapped(){
        ...
        view.layoutIfNeeded()
        self.filterMainViewHeight.constant = 180 + GeneralFunctions.getSafeAreaInsets().bottom
        ...
        if(self.currentCabGeneralType == Utils.cabGeneralType_UberX && isDriverAssigned == false && requestPickUpView == nil){
            ...
            for i in 0..<currentLoadedDriverList.count{
                ...
                if(driverId == driver_id){
                    ...
                    let triangle = TriangleView(frame: CGRect(x: 0, y: 0, width: 19.5 , height: 30))
    ...
    func setData(){
        ...
        GeneralFunctions.setImgTintColor(imgView: menuImgView, color: UIColor.UCAColor.AppThemeColor)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.moreInfoBtn.layer.shadowOpacity = 0.5
        cell.moreInfoBtn.layer.shadowOffset = CGSize(width: 0, height:2)
        cell.moreInfoBtn.layer.shadowColor = UIColor.black.cgColor
        ...
        if eIsFeatured.uppercased() == "YES"{
            cell.containerView.layer.borderColor = UIColor.UCAColor.AppThemeColor.cgColor
            cell.containerView.layer.borderWidth = 1
            cell.featuredLblContainerVw.isHidden = false
            cell.topVwDriverInfo.constant = 23
            
            //            cell.containerView.backgroundColor = UIColor(hex:0xded9df)
        }else{
        ...
        cell.featuredLbl.backgroundColor = UIColor.UCAColor.AppThemeColor
        ...
        //        cell.moreInfoBtn.tag = indexPath.item
        
        cell.moreInfoBtn.tag = GeneralFunctions.parseInt(origValue: -1, data: item.get("driver_id"))
        cell.moreInfoBtn.btnType = "UFX_DRIVER_MORE_INFO"
        cell.moreInfoBtn.setButtonTitle(buttonTitle: self.generalFunc.getLanguageLabel(origValue: "", key: "LBL_MORE_INFO"))
        cell.moreInfoBtn.clickDelegate = self
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + Double(Int64(4 * Double(NSEC_PER_SEC))) / Double(NSEC_PER_SEC), execute: {
                ...
                if(self.ufxCurrentLoadedDriverList.count == 0){
                    ...
                    if(self.UFX_PAGE_MODE == choiceType){
                        ...
                        self.ufxNoProviderView = UFXNoProviderView(frame: CGRect(x:0, y: 0, width: Application.screenSize.width, height: viewHeight))
                        self.ufxNoProviderView.setMainScreen(mainScreenUv: self)
                        self.view.addSubview(self.ufxNoProviderView)
    ...
    func getAllCenter(){
        ...
        if(isUpwardLocation == true){
            edgeInsets = UIEdgeInsets(top: self.addressBarYPosition + addressBarHeight + heightOfWaitingMarker, left: 20, bottom: (UIScreen.main.scale == 3 ? 64 : 32) + 20, right: 20)
        }else{
            edgeInsets = UIEdgeInsets(top: self.addressBarYPosition + addressBarHeight, left: UIScreen.main.scale == 3 ? 60 : 20, bottom: 20 + 20, right: 20)
        }
    ...
    func openRequestPickUpView(isFromRideLater:Bool){
        ...
        arrowPayImgView.transform = CGAffineTransform(rotationAngle: 90 * CGFloat(CGFloat.pi/180))
        var height:CGFloat = 291 + (GeneralFunctions.getSafeAreaInsets().bottom / 2)
        ...
        self.machinePayLbl.text = self.generalFunc.getLanguageLabel(origValue: "Maquininha", key: "LBL_POS")
        ...
        self.isPosPayment = false
        self.isCardPayment = false
        ...
        self.packageTypeId = ""
        self.packageDetails = ""
        self.requestNowBtn.isAppTheme = true
        
        self.payLbl.text = self.generalFunc.getLanguageLabel(origValue: "", key: "LBL_CASH_TXT")
        isPosPaymentEmpty = true
        //TODO: Fazer o tipo Cash-Pos
        if(self.userProfileJson.get("APP_PAYMENT_MODE") == "Cash-Card"){
        ...
        } else if(self.userProfileJson.get("APP_PAYMENT_MODE") == "Cash-Pos"){
            self.payView.tag = 0
            self.payView.isUserInteractionEnabled = true
            self.payView.addGestureRecognizer(getPayTapGue())
            ...
            self.machinePaymentView.tag = 3
            self.machinePaymentView.isUserInteractionEnabled = true
            self.machinePaymentView.addGestureRecognizer(getPayTapGue())
            
        } else if(self.userProfileJson.get("APP_PAYMENT_MODE") == "Cash-Card-Pos"){
            self.payView.tag = 0
            self.payView.isUserInteractionEnabled = true
            self.payView.addGestureRecognizer(getPayTapGue())
            ...
            //Marlon - Adicionada nova forma de pagamento
            self.machinePaymentView.tag = 3
            self.machinePaymentView.isUserInteractionEnabled = true
            self.machinePaymentView.addGestureRecognizer(getPayTapGue())
            self.isCardPayment = true
        ...
        }else if(self.userProfileJson.get("APP_PAYMENT_MODE") == "Pos"){
            isCardPayment = false
            isCashPayment = false
            isPosPayment = true
            self.payLbl.text = self.generalFunc.getLanguageLabel(origValue: "", key: "LBL_POS")
            self.payImgView.image = UIImage(named: "ic_machine_card")
        }else{
            isCashPayment = true
            isCardPayment = false
            isPosPayment = false
    ...
    func addPickUpMarkerWithTimeLbl(){
        ...
        timeLbl.backgroundColor = UIColor.UCAColor.AppThemeColor
    ...
    func addMarker(){
        ...
        GeneralFunctions.setImgTintColor(imgView: pickUpPin, color: UIColor.UCAColor.AppThemeColor)
        ...
        GeneralFunctions.setImgTintColor(imgView: dropPin, color: UIColor.UCAColor.AppThemeColor)
    ...
    @objc func rideLaterImgTapped(){
        ...
        if((self.userProfileJson.get("APP_PAYMENT_MODE") == "Card" && isCardValidated == false) || (isCashPayment == false && isCardValidated == false) || (isPosPayment == false && isCardValidated == false)){
            self.checkCardMode = ""
    ...
    func checkFlatFareExist(isRideLater:Bool, dataDict:NSDictionary){
        ...
        if(dataDict.get("eFlatTrip").uppercased() == "YES"){
            ...
            //Marlon - Retirado o aviso de tarifa
            //if(dataDict.get("Action") == "1"){
    ...
    @objc func paymentOptionTapped(sender:UITapGestureRecognizer){
        let payOptionsHeight = CGFloat(self.userProfileJson.get("APP_PAYMENT_MODE") == "Cash-Card-Pos" ? 80 : 40)
        if(sender.view!.tag == 0){
            
            self.requestPickUpView.frame.size = CGSize(width: self.requestPickUpView.frame.width, height: self.requestPickUpView.frame.height + payOptionsHeight)
            self.requestPickUpView.frame.origin.y = self.cntView.frame.height - self.requestPickUpView.frame.height
            
            UIView.animate(withDuration: 0.5,
                           animations: {
                            self.cardPaymentView.isHidden = self.isCardPayment ? false : true
                            self.cardViewHeight.constant = self.isCardPayment ? 40 : 0
                            self.payViewHeight.constant = 40
                            self.cashPaymentView.isHidden = false
                            self.machinePaymentView.isHidden = false
                            self.payView.isHidden = true
                            self.promoCodeView.isHidden = true
                            
                            self.paymentOptionContainerViewHeight.constant = self.paymentOptionContainerViewHeight.constant + payOptionsHeight
                            self.paymentOptionContainerView.frame.size = CGSize(width: self.paymentOptionContainerView.frame.width, height: self.paymentOptionContainerView.frame.height + payOptionsHeight)
                            
                            self.paymentContainerView.frame.size = CGSize(width: self.paymentOptionContainerView.frame.width, height: self.paymentOptionContainerView.frame.height + payOptionsHeight)
            ...
            UIView.animate(withDuration: 0.5,
                           animations: {
                            ...
                            self.machinePaymentView.isHidden = true
            ...
            if(sender.view!.tag == 1){
                ...
                isCashPayment = false
                isPosPayment = true
                self.payLbl.text = self.generalFunc.getLanguageLabel(origValue: "Maquininha", key: "LBL_POS")
                self.payImgView.image = UIImage(named: "ic_card_machine")
    ...
    func checkCardConfig(isRideLater:Bool, isAutoContinue:Bool){
        ...
        if(userProfileJson.get("vStripeCusId") != "" || userProfileJson.get("vBrainTreeCustId") != "" || userProfileJson.get("vPaymayaCustId") != "" || userProfileJson.get("vOmiseCustId") != "" || userProfileJson.get("vAdyenToken") != "" || userProfileJson.get("vXenditToken") != "" || userProfileJson.get("vBepayToken") != ""){
            showPaymentBox(isRideLater:isRideLater, isAutoContinue: isAutoContinue)
            //Marlon - Adicionada verificação de pagamento com Zoop
        } else if (userProfileJson.get("APP_PAYMENT_METHOD") == "Zoop") {
            let vZoopAccountId = userProfileJson.get("vZoopAccountId")
            if (vZoopAccountId.isEmpty) {
                changeCard()
            } else {
                showPaymentBox(isRideLater: isRideLater, isAutoContinue: isAutoContinue)
            }
    ...
    func retryReqBtnPressed(){
        let vPaymentMode = isPosPaymentEmpty ? "" : isCashPayment ? "Cash" : isPosPayment ? "Pos" : "Card"
        var parameters = ["type":"loadAvailableCab","PassengerLat": "\(self.pickUpLocation!.coordinate.latitude)", "PassengerLon": "\(self.pickUpLocation!.coordinate.longitude)","iUserId": GeneralFunctions.getMemberd(), "PickUpAddress": self.pickUpAddress, "eType": currentCabGeneralType , "SelectedCabType": currentCabGeneralType , "sortby" : selctedFilterType, "vPaymentMode": vPaymentMode ]
        if(ufxSelectedVehicleTypeId != ""){
            parameters["iVehicleTypeId"] = ufxSelectedVehicleTypeId
            if(selectedDate != ""){
                parameters["scheduleDate"] = selectedDate
            }
        }
        if(eShowOnlyMoto == true){
            parameters["eShowOnlyMoto"] = "Yes"
        }
        if((selectedCabCategoryType == Utils.rentalCategoryType || eShowOnlyRental == true)){
            parameters["eRental"] = "Yes"
        }
        let exeWebServerUrl = ExeServerUrl(dict_data: parameters, currentView: view, isOpenLoader: false)
        ...
        exeWebServerUrl.executePostProcess(completionHandler: { (response) -> Void in
            ...
            if(response != ""){
                ...
                for i in 0..<cabListArr.count {
                    ...
                    if(tempDict != nil){
                        id = id + (tempDict.get("iDriverId") + ",")
                    }
                }
                
                self.currentLoadedDriverIdsArr = id.components(separatedBy: ",")
    ...
    func myBtnTapped(sender: MyButton) {
        if(self.requestNowBtn != nil && sender == self.requestNowBtn){
             ...
             */
            if((self.userProfileJson.get("APP_PAYMENT_MODE") == "Card" && isCardValidated == false) || (isCashPayment == false && isCardValidated == false && isPosPayment == false)){
                self.checkCardMode = ""
    ...
    func openOutStandingAmountBox(isFromRideNow:Bool){
        ...
        self.outStandingAmtValueLbl.text = Configurations.convertNumToAppLocal(numStr: self.userProfileJson.get("fOutStandingAmountWithSymbol"))
        ...
        self.adjustAmtLbl.text = self.generalFunc.getLanguageLabel(origValue: "", key: self.currentCabGeneralType.uppercased() == Utils.cabGeneralType_Ride.uppercased() ? "LBL_ADJUST_OUT_AMT_RIDE_TXT" : (self.currentCabGeneralType.uppercased() == Utils.cabGeneralType_UberX.uppercased() ? "LBL_ADJUST_OUT_AMT_JOB_TXT" : "LBL_ADJUST_OUT_AMT_DELIVERY_TXT"))
        ...
        if(Configurations.isRTLMode()){
            payNowImgView.transform = CGAffineTransform(scaleX: -1, y: 1)
            adjustAmtImgView.transform = CGAffineTransform(scaleX: -1, y: 1)
        }
    ...
    func configureAssignedDriver(isAppRestarted:Bool){
        ...
        if(isAppRestarted == true){
            ...
            if (vTripPaymentMode == "Cash") {
                self.isCashPayment = true
                self.isPosPayment = false
            } else if (vTripPaymentMode == "Pos") {
                self.isCashPayment = false
                self.isPosPayment = true
            }else {
                self.isCashPayment = false
            }
            assignedDriverId = tripDetailJson.get("iDriverId")
        ...
        exeWebServerUrl.executePostProcess(completionHandler: { (response) -> Void in
            if(response != ""){
                ...
                if(dataDict.get("Action") == "1"){
                    ...
                    //Marlon - Possível mensagem de erro de requisição em caso de pagamento com Zoop
                    if (dataDict.get(Utils.message_str) == "LBL_GATEWAY_COMMUNICATION_ERROR" || dataDict.get(Utils.message_str) == "LBL_CREDIT_CARD_ERROR" || dataDict.get(Utils.message_str) == "LBL_INVALID_REQUEST" || dataDict.get(Utils.message_str) == "LBL_INSUFFICIENT_FUNDS" || dataDict.get(Utils.message_str) == "LBL_TRANSACTION_REFUSED" || dataDict.get(Utils.message_str) == "LBL_CARD_CUSTOMER_NOT_ASSOCIATED") {
                        
                        self.generalFunc.setAlertMessage(uv: self, title: "", content: dataDict.get(Utils.message_str), positiveBtn: self.generalFunc.getLanguageLabel(origValue: "Ok", key: "LBL_BTN_OK_TXT"), nagativeBtn: "", completionHandler: { (btnClickedIndex) in
    ...
    func setTripStartData(deliveryConfirmCode:String){
        ...
        self.emeImgView = UIImageView(frame: CGRect(x: Configurations.isRTLMode() ? (self.view.frame.width - 15) : 15,y: self.addressContainerView.frame.maxY + 10, width: 50, height: 50))
    ...
    func requestCab(tollPrice:String, tollPriceCurrencyCode:String, isTollSkipped: String, _ isFromWalletChk:Bool = false){
        ...
        if(userProfileJson.get("eWalletBalanceAvailable").uppercased() == "YES" && isFromWalletChk == false){
    ...
    func noDriverAvailView(isHidden:Bool){
        if(requestCabView != nil){
            reqNoteLbl.isHidden = true
            retryReqBtn.isHidden = true
            
            if(isHidden == false){
                
                self.retryReqBtnPressed()
    ...
    func continueRideLaterSchedule(tollPrice:String, tollPriceCurrencyCode:String, isTollSkipped:String, _ isFromWalletChk:Bool = false){
    ...
    func sendRequestToDrivers(driverIds:String, tollPrice: String, tollPriceCurrencyCode: String, isTollSkipped: String){
        ...
        exeWebServerUrl.executePostProcess(completionHandler: { (response) -> Void in
            ...
            if(response != ""){
                ...
                if(dataDict.get("Action") != "1"){
                    if(dataDict.get(Utils.message_str) == "LBL_TRANSACTION_REFUSED" || dataDict.get(Utils.message_str) == "LBL_GATEWAY_COMMUNICATION_ERROR" || dataDict.get(Utils.message_str) == "LBL_CREDIT_CARD_ERROR" || dataDict.get(Utils.message_str) == "LBL_INVALID_REQUEST" || dataDict.get(Utils.message_str) == "LBL_INSUFFICIENT_FUNDS" || dataDict.get(Utils.message_str) == "LBL_CARD_CUSTOMER_NOT_ASSOCIATED" || dataDict.get(Utils.message_str) == "LBL_ALREADY_REGISTERED_CPF") {
                        self.stopDriverRequestQueue()
                        self.closeCabReqView()
                        ...
                        return
                    } else if(dataDict.get(Utils.message_str) == "SESSION_OUT"){
                        self.closeCabReqView()
                        self.stopDriverRequestQueue()
                        GeneralFunctions.logOutUser()
                        self.generalFunc.setAlertMessage(uv: self, title: self.generalFunc.getLanguageLabel(origValue: "", key: "LBL_SESSION_TIME_OUT"), content: self.generalFunc.getLanguageLabel(origValue: "Your session is expired. Please login again.", key: "LBL_SESSION_TIME_OUT"), positiveBtn: self.generalFunc.getLanguageLabel(origValue: "Ok", key: "LBL_BTN_OK_TXT"), nagativeBtn: "", completionHandler: { (btnClickedIndex) in
                    ...
                    if(self.currentCabGeneralType.uppercased() != Utils.cabGeneralType_Ride.uppercased()){
                        if(dataDict.get(Utils.message_str) == "NO_CARS" || dataDict.get(Utils.message_str) == "LBL_PICK_DROP_LOCATION_NOT_ALLOW" || dataDict.get(Utils.message_str) == "LBL_DROP_LOCATION_NOT_ALLOW" || dataDict.get(Utils.message_str) == "LBL_PICKUP_LOCATION_NOT_ALLOW"){
                            self.stopDriverRequestQueue()
                            self.closeCabReqView()
