...
class AddPaymentUV: UIViewController, MyBtnClickDelegate, UIWebViewDelegate {
    ...
    @IBOutlet weak var cardHImgView: UIImageView!
    ...
    override func viewDidLoad(){
        ...
        if(payMentMethod == "Adyen" || payMentMethod == "Bepay" || payMentMethod == "Zoop"){
            self.addTokenToServer(token: "", vXenditAuthId: "")
        }
    ...
    func setData(){
        ...
        if(self.payMentMethod == "Bepay"){
            self.cardHImgView.isHidden = true
            self.creditCardNumView.isHidden = true
            self.expiryView.isHidden = true
            self.cvvView.isHidden = true
            self.configCardBtn.isHidden = true
        }
    }
    ...
    func checkData(){
        ...
        DispatchQueue.main.async() {
            ...
            }else if(self.payMentMethod == "Bepay" || self.payMentMethod == "Zoop"){
                self.addTokenToServer(token: "", vXenditAuthId: "")
            }else if(self.payMentMethod == "Xendit"){
                self.generateTokenUsingXendit()
            }else{
                self.generateToken()
            }
        }
    ...
    func generateTokenUsingXendit(){
        Xendit.publishableKey = self.userProfileJson.get("XENDIT_PUBLIC_KEY")
        let cardData = CardData()
        cardData.cardNumber = Utils.getText(textField: self.creditCardTxtField.getTextField()!).trimAll()
        cardData.cardExpMonth = Utils.getText(textField: self.monthTxtField.getTextField()!)
        cardData.cardExpYear = Utils.getText(textField: self.yearTxtField.getTextField()!)
        cardData.cardCvn = Utils.getText(textField: self.cvvTxtField.getTextField()!)
        cardData.isMultipleUse = true
        
        loadingDialog = NBMaterialLoadingDialog.showLoadingDialogWithText(self.contentView, message: (GeneralFunctions()).getLanguageLabel(origValue: "Loading", key: "LBL_LOADING_TXT"))
        ...
        Xendit.createToken(fromViewController: self, cardData: cardData, shouldAuthenticate: true) { (token, error) in
            ...
            DispatchQueue.main.async() {
                ...
                if(error != nil){
                    self.generalFunc.setError(uv: self, title: "", content: error!.message)
                }else{
                    self.addTokenToServer(token: token!.id, vXenditAuthId: token!.authenticationId != nil ? token!.authenticationId : "")
                }
            }
        }
    }
    ...
    func generateToken(){
        ...
        cardParams.expMonth = UInt(GeneralFunctions.parseInt(origValue: 0, data: Utils.getText(textField: self.monthTxtField.getTextField()!)))
        cardParams.expYear = UInt(GeneralFunctions.parseInt(origValue: 0, data: Utils.getText(textField: self.yearTxtField.getTextField()!)))
        ...
        STPAPIClient.shared().createToken(withCard: cardParams) { (token, error) in
            if error != nil {
                ...
                self.addTokenToServer(token: token.tokenId, vXenditAuthId: "")
            }
    ...
    func gemneratePaymayaToken(){
        ...
        PayMayaSDK.sharedInstance()?.createPaymentToken(from: cardParams, result: { (results, error) in
            //let result = results.
            DispatchQueue.main.async {
                self.loadingDialog.hideDialog()
            }
        ...
        PayMayaSDK.sharedInstance()!.createPaymentToken(from: cardParams!){ (results, error) in
            ...
            if(results?.status == PMSDKPaymentTokenStatus.created){
                
                var tokenObj = PMSDKPaymentToken.init()
                tokenObj = (results?.paymentToken)!
                
                self.payMayaToken = tokenObj.identifier
                
                DispatchQueue.main.async {
                    self.addTokenToServer(token: tokenObj.identifier, vXenditAuthId: "")
                }
            }else{
                self.generalFunc.setError(uv: self)
            }
    ...
    func addTokenToServer(token:String, vXenditAuthId:String){
        ...
        let parameters = ["type":"GenerateCustomer","iUserId": GeneralFunctions.getMemberd(), "UserType": Utils.appUserType]
        ...
        exeWebServerUrl.executePostProcess(completionHandler: { (response) -> Void in
            ...
            if(response != ""){
                ...
                if(dataDict.get("Action") == "1"){
                    ...
                    if(self.payMentMethod == "Paymaya" || self.payMentMethod == "Adyen" || self.payMentMethod == "Bepay" || self.payMentMethod == "Zoop"){
                        self.webView.isHidden = false
                        
                        DispatchQueue.main.async {
                            self.activityIndicator.startAnimating()
                        }
                        print(Utils.message_str)
